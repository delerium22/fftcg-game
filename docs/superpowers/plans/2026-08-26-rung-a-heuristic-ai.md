...
### Task 4b: Codex plan-review fix wave (code built in Tasks 1–4)

Codex's adversarial plan review (`docs/superpowers/plans/2026-08-26-rung-a-heuristic-ai.codex-review.md`, run after Tasks 1–3 were built) found defects in the already-implemented code. One implementer applies all of them; one workflow review gates the wave.

**Files:** `packages/engine/src/determinise.ts`, `packages/ai/src/{evaluate,payment,candidates,greedy,agent}.ts`, their tests; `packages/engine/test/determinise.test.ts`.

- [ ] **F1 — Rollouts must fight (Codex HIGH "depth-2 rollouts do not model opponent attacks").** `greedyStep` scores `apply(declareAttack)` immediately: the attacker is dull and no damage has happened yet, so `pass` always wins and simulated turns never attack. Fix in `greedy.ts`: add `resolveCombat(state, weights, aggression): GameState` — while `!state.result && (state.pending?.kind === 'declareBlock' || state.pending?.kind === 'assignPartyDamage')`, let the acting player answer with `greedyStep` from THEIR perspective (`p === original decider ? aggression : 1 − aggression` is not knowable here — use `aggression` for the state's `turnPlayer` and `1 − aggression` for the other player, passed in as `perspective: PlayerId`) and apply it. `greedyStep` scores every candidate on `resolveCombat(apply(state, c).state)`; `decide` likewise resolves combat after each top-level candidate before rolling out. Recursion terminates because those pending kinds resolve within two steps. Test (`greedy.test.ts`): with my 3000 forward alone on my side and the opponent holding an active 7000 forward and no blockers on my side, a depth-2 decision at MY attack declaration must prefer `pass` AND `lastDepth === 2`; and a direct unit test of `greedyStep` for the opponent at THEIR attack declaration with a 7000 forward against my empty board must return a `declareAttack`, not `pass`.
- [ ] **F2 — Honest simulation budget (Codex HIGH "maxSimulations is not a cap").** Count every `apply` made while scoring (inside `greedyStep` and `resolveCombat`) against the per-candidate `used`; keep the floor of one top-level apply per candidate (so `lastSimulations ≤ maxSimulations + cands.length`), document that bound on `GreedyOptions.maxSimulations`, and if `cands.length > maxSimulations` keep only the first `maxSimulations` candidates deterministically (pass stays last). Test: `lastSimulations <= 6 + lastCandidates` with `maxSimulations: 6`.
- [ ] **F3 — Bounded attack candidates (Codex HIGH "attack generation is exponential").** In `candidates.ts`, replace the direct `legalAttackSets` call with `boundedAttackSets(state, player)`: if the number of attack-eligible forwards is ≤ 6 use `legalAttackSets`; otherwise emit every single plus, per element, the full party of all eligible forwards sharing that element (deduplicated). Test with 8 eligible forwards: candidates ≤ 8 + elements, all legal via `attackCheck === null`. (Engine's `1 << n` mask in `legalAttackSets` stays as a backlog item — unreachable with a 50-card deck.)
- [ ] **F4 — `preferredPayment` scarcity order (Codex HIGH).** Satisfy required elements scarcest-first (fewest matching sources), and when choosing a source for an element prefer the least flexible one (fewest elements) among equal-cost candidates, so a dual-element discard is kept for the element that needs it. Add Codex's counterexample as a test: Earth/Lightning cost-3 target, a cheap Earth+Lightning dual discard, an expensive Earth-only discard, plus one Earth backup → payment must exist.
- [ ] **F5 — Setup decisions at depth 0 (Codex HIGH "chooseFirst horizons").** In `decide`, when `det.phase === 'setup'` use depth 0 (the evaluation already scores hand quality) — `lastDepth` reports 0. Test: from `createGame` (pending chooseFirst) both candidates are scored without a rollout (`lastSimulations === lastCandidates`).
- [ ] **F6 — Aggression contract (Codex MEDIUM).** Move active power into `material()` as a per-side tempo feature (`Σ active forward power/1000 · w.threat`), delete the separate antisymmetric term. Zero-sum at 0.5 still holds (pure per-side difference). Tests: aggression 0 result is invariant when ONLY the opponent's board changes; aggression 1 invariant when only mine changes; `evaluate` throws on aggression outside [0,1]. Rename nothing (keep `threat` as the weight name; document it as "active-power tempo").
- [ ] **F7 — Test strengthening (Codex MEDIUM/LOW).** `evaluate.test.ts` handQuality: compare two equal-size hands (V-F1 vs V-F8). `determinise.test.ts`: assert `viewFor(det, me)` deep-equals `view` entirely (drop the `cards: null` masking). `greedy.test.ts`: determinism over a whole game — two fresh agents with the same seed driving the same seeded game produce identical command traces (use a small maxSimulations to keep it fast). `decide` resets `lastSimulations/lastCandidates/lastDepth` at entry and throws if no command exists at all.
- [ ] **F8 — determinise hardening (Codex MEDIUM).** Synthetic ids start at `max(SYNTHETIC_ID_BASE, maxVisibleId + 1)`; every deck-list code must exist in `view.defs` (throw mentioning "deck list" otherwise). Document at the API boundary that supplying deck lists is a game-mode assumption (open/fixed lists, e.g. the Vol. 2 starter matchup) and that callers must never derive them from hidden state.
- [ ] Run everything, commit as `fix(ai): fight in rollouts, honest budget, bounded attacks, scarcity-first payment, setup depth 0, per-side tempo` (one or several commits, each green, each with the trailer).
...
```
`strict` (default `true`) keeps the per-command mutation/invariant/dead-end checks; `strict: false` skips them for tournaments (still runs `checkInvariants` once at game end). **Timing is end-to-end per decision**: `performance.now()` around view construction + legal-command generation + `decide` (Codex: timing only `decide` hid the `legalCommands`/`enumeratePayments` cost). To keep that cost off the greedy path, `Agent` gains an optional `needsLegalCommands?: boolean` (RandomAgent `true`; GreedyAgent `false`): self-play passes `[]` as `legal` when it is `false`, and `GreedyAgent.decide` computes `legalCommands(det, me)` itself only in its fallback branch. Seat seeds: game `g` uses `seed + g`; the agent for seat `p` is constructed fresh per game with seed `(seed + g) * 2 + p + 1` (documented in `agents.ts`). `main.ts`: `--p0`, `--p1` (default `random`), `--depth` (default 1, applied to greedy specs without an explicit depth), `--fast` → `strict: false` — the existing `flag(name, dflt)` helper reads a VALUE after the flag, so add `const has = (name: string) => rest.includes(`--${name}`)` for boolean flags; the printed JSON includes `agents` and `msPerDecision`. `makeAgent` builds `GreedyOptions` without undefined-valued keys when `spec.depth` is absent (or relies on the `| undefined` declarations).
...
```
Also assert in the existing 20-game test that `r.agents` equals `['random', 'random']`. Add `apps/cli/test/agents.test.ts`: `parseAgentSpec` accepts `random`, `greedy`, `greedy:0`, `greedy:2` and throws on `greedy:3` / `foo`; `describeAgentSpec` round-trips; `makeAgent` returns a `RandomAgent` / `GreedyAgent` with `needsLegalCommands` false for greedy; and a determinism test: two `selfPlay` runs with identical options produce identical reports once `msPerDecision` is removed.
...
- [ ] **Step 1:** Tune on a SEPARATE seed range (`--seed 1000`), report the gate on the untouched range (`--seed 1`) — Codex: never tune and report on the same seeds. Run the three 200-game tournaments from Task 5 Step 4 at depth 1 and depth 2 (`--depth 2`) on seed 1000. If greedy < 80 % vs random in either seat at depth 1: adjust `DEFAULT_WEIGHTS` (try, in order: `damage` 30→40, `threat` 0.8→1.5, `forwardPresence` 4→6), re-run on seed 1000, keep the best; at most 4 iterations; record every run's numbers. Then run the seed-1 gate once with the final weights. Any weight change must keep the Task 2/4 tests green.
- [ ] **Step 2:** Append `## Appendix — rung A measurements (2026-08-26)` to the spec: final weights and a table of win rate / msPerDecision (end-to-end) / avgTurns for greedy-vs-random (both seats), greedy-vs-greedy, at depth 1 and 2, on the seed-1 gate range; label the baseline as *concrete-command random* (RandomAgent samples uniformly over payment permutations, which over-weights castable cards — Codex). Also reconcile the spec's interface sections with what was built (`determinise({ view, decks, rng }): [GameState, Rng]`, `preferredPayment(state, player, card)`, `AgentSpec` without seeds, `needsLegalCommands`), and note the open-deck-list fairness assumption (A3).
