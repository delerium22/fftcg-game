...
- **A4**: the built signature is `preferredPayment(state, player, card)` — it operates on a full (determinised) `GameState`, not a bare `PlayerView`, because it needs backup/hand card definitions and `canPay`/`generateCp` from the engine. It also resolves multi-element costs by paying the *scarcest* required element first (fewest matching sources), not simply "dull backups then discard" in element order — the naive order can strand itself when a dual-element source is spent on an element a plentiful source could have covered.
...
packages/ai/src/greedy.ts            class GreedyAgent implements Agent; greedyStep, resolveCombat, pruneCandidates (exported for testing)
...
2. Build candidates via `candidateCommands(det, me)` — legal commands with cast commands collapsed to one per card via `preferredPayment` (bypassing `legal`'s enumerated payment permutations), and attack declarations bounded above 6 eligible attackers (`boundedAttackSets`: every singleton plus, per element, the full same-element party) instead of `legalAttackSets`'s full 2ⁿ enumeration. `pass` is always last. `legal` is only consulted as a fallback (empty candidates ⇒ pick the first non-concede `legal`/`legalCommands(det, me)` command) — the hot path never needs it, which is what `needsLegalCommands: false` signals to the caller.
...
5. For each candidate: apply it, then `resolveCombat` fast-forwards any pending `declareBlock`/`assignPartyDamage` (both sides answered by `greedyStep` at the appropriate perspective) so combat is always resolved before scoring — never scored mid-combat. Then, per depth: `depth ≥ 1` rolls out the rest of the *acting turn's owner's* turn with `greedyStep`; `depth ≥ 2` also rolls out the following (opponent's) turn. Score the resulting state with `evaluate(s, me, weights, aggression)`; keep the first candidate with the strictly best score (ties favor the earlier candidate, which is `pass`-last by construction).
6. Budget: a shared `Budget { used, cap: maxSimulations }` is charged for every `apply()` anywhere in scoring/rollout/combat-resolution (not just the winning line), but every top-level candidate is still guaranteed at least one apply — so `lastSimulations ≤ maxSimulations + lastCandidates`. `lastSimulations`, `lastCandidates`, `lastDepth` are exposed for tests/diagnostics.
...
| greedy vs greedy | 2 | 102 / 98 | — | 200/200 | 18.530 | 0.3578 / 0.3699 |
