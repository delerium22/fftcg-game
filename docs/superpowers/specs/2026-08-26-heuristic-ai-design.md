...
**Re-verified again after the fix wave's own re-review (2026-08-27, `DEFAULT_WEIGHTS` unchanged, depth 1, same `--seed 1` gate)**: greedy(p0) vs random **200/0** (100 %); random vs greedy(p1) **2/198** (99 %). All 200/200 completed, 0 failures; 0.27 / 0.28 ms per greedy decision. Both seats improved by one game over the row above — the re-review's R1 fix (`preferredPayment` ranks required-element sources by card value again, not by CP generated) stops the agent discarding a high-power forward where a cost-1 summon would pay the same cost, so it is a small genuine strength gain rather than seed noise. A 40-game greedy-vs-greedy mirror under full strict invariant checking finished 21/19 with 0 failures.
