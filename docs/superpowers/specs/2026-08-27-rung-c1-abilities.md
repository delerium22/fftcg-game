...
| C1-9 | Coverage is tracked **per clause** | Each clause gets a stable `abilityId`; a card warns `unimplementedAbility` for exactly the clauses that are not implemented. Noel, Cloud and Ramuh keep warning about their unimplemented clauses even in C1. The log stays honest, which is what B-A6 promised the player. |
...
- **C1-A2** `unimplementedAbility` is emitted for exactly the **clauses** that remain unimplemented, and
  no longer for the five that are. Noel, Cloud and Ramuh still warn about their other clauses.
