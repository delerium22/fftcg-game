...
|---|---|---|
| C3-1 | **`activateAbility { player, source, abilityId, payment }` — targets PREFLIGHTED, not carried** | The problem is real: `resolve.ts:235` treats an empty target set as a successful no-op, which is right for a triggered ability and wrong here — it would let Undead Princess pay her whole cost, breaking herself, for nothing. The plan review's fix was to declare targets *with* the activation. **Implementation went the other way**, and this row records the change rather than hiding it: activation computes the ability's first `chooseTargets` candidate set against the state as it will be **once the costs are paid**, and is illegal unless a legal target exists. Target choice then stays an ordinary `Pending`, exactly as it already is for every triggered ability. Preflighting post-cost is what makes it exact, and two things fall out for free: Undead Princess has already left the field so she cannot be her own target, and the set the player is offered afterwards is by construction the set that was preflighted — no second code path to keep in agreement. **Known limit:** it inspects only the *first* effect, so an ability whose targeting is nested or repeated would not be fully preflighted. None of the six clauses is, and the C3 code review was asked to attack exactly this. |
